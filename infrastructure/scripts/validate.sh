...
if [ "$MODE" = "--what-if" ]; then
  ...
  az deployment group what-if \
    ...
    --parameters environmentName="$ENVIRONMENT" location="$LOCATION" openAiApiKey="${OPENAI_API_KEY:-set-in-github-actions}" appSessionSigningKey="${APP_SESSION_SIGNING_KEY:-set-in-github-actions}" appleClientId="${APPLE_CLIENT_ID:-com.simonholmes.voxa}" appleTeamId="${APPLE_TEAM_ID:-2PA85SU4UQ}" appleKeyId="${APPLE_KEY_ID:-set-in-github-actions}" applePrivateKey="${APPLE_PRIVATE_KEY:-set-in-github-actions}" networkResourceGroupName="$NETWORK_RESOURCE_GROUP"
