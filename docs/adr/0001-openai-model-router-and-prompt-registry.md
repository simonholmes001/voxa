...
- Every prompt carries a required schema: `id`, `version`, `capability`, `variables`, `system`, `user`, `outputSchema`, `guardrails`.
- Committed versions are **immutable**; behavioral edits require bumping `version`.
- A build-time `router-index.json` records each version's `sha256` over its behavioral fields; CI fails any behavioral drift on a committed version.
